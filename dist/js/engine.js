import {installGem,equippedSkills} from './socket-model.js';
import {dropGem,pickupLoot} from './ground-loot.js';
...
export class Game{
 ...
 install(index,socket,gem){const r=installGem(this,index,socket,gem);if(r.ok){refreshAuras(this);this.emit('loadout')}return r}
 ...
 finish(){if(this.state!=='forge'||this.round!==3||this.wave!==15||!this.rewardTaken)return false;this.state='won';this.emit('end',true);return true}
 ...
 spawnWave(){if(this.enemies.length>=MAX_ENEMIES)return;const p=this.player,n=Math.min(this.waveRemaining,packSize(this.round));for(let i=0;i<n&&this.enemies.length<MAX_ENEMIES;i++){const a=this.rng()*Math.PI*2,r=350+this.rng()*150,x=clamp(p.x+Math.cos(a)*r,65,WORLD_SIZE-65),y=clamp(p.y+Math.sin(a)*r,65,WORLD_SIZE-65),roll=this.rng();const pool=enemyPool(this.round,this.wave);const type=pool[Math.floor(roll*pool.length)];this.spawn(type,x,y);this.waveRemaining--}}
 ...
 shoot(){return castSkill(this,compileSkill(equippedSkills(this)[0],this.player,this.gemLevels))}
 ...
 updateHealingPools(this,dt);pickupLoot(this);
