...
export function drawLottery(g){
 ...
 g.tickets--;let reward;
 if(gems.length&&g.rng()>=.5){const id=gems[Math.floor(g.rng()*gems.length)];g.bag.support.push(id);reward={kind:'gem',...SUPPORT_GEMS[id],id}}
 else{const w=makeLegendary(g,Math.floor(g.rng()*LEGENDARY_WEAPONS.length));stashEquipment(g,w);reward={kind:'weapon',...w}}
 g.lastPrize=reward;g.emit('loadout');g.emit('sound','win');return {ok:true,reason:'臨界降臨：'+reward.name,reward};
}
