...
export function castSkill(game,s){
 const p=game.player,target=nearest(game,p,s.id==='nova'?s.radius+40:600);
 if(s.id==='orbit'){
...
export function updateCasting(game,dt){
 for(let i=0;i<game.links.length;i++){
   game.castClocks[i]-=dt;
   const s=compileSkill(game.links[i],game.player,game.gemLevels);
   if(game.castClocks[i]<=0&&castSkill(game,s)){
     game.castClocks[i]=1/s.rate;
...
export function updateProjectiles(game,dt){
 ...
 for(const s of game.shots){
   ...
   if(game.world.hash.query(s.x,s.y,50).some(o=>distance(s,o)<o.r+s.r)){s.life=0;game.burst(s.x,s.y,s.color||'#e8b474',3);continue}
